...
/*
  Creates a height x width position matrix and initiallize it to POS_NOTHING.
 */
position **new_board(unsigned int height, unsigned int width){
  ...
  data = malloc(width*height*sizeof(position));
 ...
 */
void add_position_type(t_config *config, unsigned int x, unsigned int y, position type){
...
void init_config_board(t_config *config, unsigned int height, unsigned int width){
  ...
  int i ;
  position **m = new_board(height, width);
...
void print_positions_of_type(t_config *config, position type){
...
void print_board_config(t_config *config){
  ...
  print_positions_of_type(config,POS_OBJECTIVE);
  printf("\n") ; 

  printf("Holes         : %u ",config->nb_holes);
  print_positions_of_type(config,POS_HOLE);
  printf("\n") ; 
...
void parse_board(FILE *f, t_config *config, struct token *current){
  ...
  for (y = height-1 ; y >= 0 ; y--){
	...
	break;
      ...
      case PERCENT:
	(config->nb_holes)++;
	add_position_type(config, x, y, POS_HOLE);
	break;

      case DOLLAR:
	(config->nb_objectives)++;
	add_position_type(config, x, y, POS_OBJECTIVE);
	break;
	...
	start_met = 1;
	add_position_type(config, x, y, POS_START);
	break ;
